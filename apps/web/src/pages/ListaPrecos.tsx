...
import { Alerta, Botao, Cabecalho, classesBotao, Input, Linha, LinhaVazia, Marcador, Select, Tabela, Td, TextoSuave, Th, Titulo, Vazio } from '../components/ui';
...
export function ListaPrecos() {
  ...
  const parametros = new URLSearchParams({ tabelaPrecoId: tabela?.id ?? '', q: busca, comPreco: String(soComPreco), porPagina: String(Math.min(quantidade, 100)) });
      ...
      {tabelas.data && ativas.length === 0 ? (
        ...
        <>
          <div className="grid gap-3 md:grid-cols-[14rem_1fr_auto] md:items-center">
            <Select aria-label="Tabela de preço" value={tabela?.id ?? ''} onChange={(e) => (setTabelaId(e.target.value), setQuantidade(POR_PAGINA))}>
              {ativas.map((t) => (
            ...
            <tbody>
              {lista.data?.itens.map((i) => (
                <Linha key={i.materialId} className="hover:bg-superficie-alt">
                  ...
                  <Td>
                    <Link to={`/materiais/${i.materialId}?aba=precos`} className="text-texto hover:text-primaria hover:underline">
                      {i.descricao}
                    </Link>
                  ...
                  <Td className="text-right whitespace-nowrap">
                    {i.precoCentavos != null ? (
                      <>
                        ...
                        {i.vigenteAte && <div className="text-xs text-texto-suave">até {formatarDataIso(i.vigenteAte)}</div>}
                      </>
                    ) : (
                  ...
                  <Td suave className="whitespace-nowrap text-sm">
                    {i.proximoPrecoCentavos != null ? `${formatarMoeda(i.proximoPrecoCentavos)} a partir de ${formatarDataIso(i.proximoInicio)}` : '—'}
                  </Td>
                  {verEstoque && (
                    <Td className={`text-right font-semibold whitespace-nowrap ${i.disponivel === 0 ? 'text-alerta' : ''}`}>
                      {formatarQuantidade(i.disponivel ?? 0)} <span className="text-xs font-normal text-texto-suave">{i.unidade}</span>
                    </Td>
                  )}
                </Linha>
              ))}
              {lista.data?.itens.length === 0 && <LinhaVazia colunas={verEstoque ? 5 : 4}>{busca ? 'Nenhum material encontrado.' : 'Nenhum material ativo nesta lista.'}</LinhaVazia>}
            </tbody>
          </Tabela>
          {lista.data && lista.data.total > 0 && (
            <div className="flex flex-col items-center gap-3">
              <TextoSuave className="text-xs">
                Mostrando {lista.data.itens.length} de {lista.data.total.toLocaleString('pt-BR')} material(is){verEstoque && ' · disponível somado de todos os depósitos'}
              </TextoSuave>
              {lista.data.total > lista.data.itens.length && quantidade < 100 && (
                <Botao variante="secundario" disabled={lista.isFetching} onClick={() => setQuantidade((q) => q + POR_PAGINA)}>
                  Mostrar mais
                </Botao>
              )}
            </div>
