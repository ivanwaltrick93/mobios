...
function saudacao() {
  const hora = Number(new Date().toLocaleString('pt-BR', { hour: '2-digit', hour12: false, timeZone: 'America/Sao_Paulo' }));
  return hora < 12 ? 'Bom dia' : hora < 18 ? 'Boa tarde' : 'Boa noite';
}

function CartaoIndicador({ indicador }: { indicador: Indicador }) {
  const disponivel = indicador.valor !== null;
  const valor = !disponivel ? '—' : indicador.formato === 'moeda' ? formatarMoeda(indicador.valor!) : indicador.valor!.toLocaleString('pt-BR');
  const conteudo = (
  ...
  return (
    <div className="space-y-8">
      ...
      <section aria-label="Indicadores" className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {painel.data?.indicadores.map((i) => <CartaoIndicador key={i.id} indicador={i} />)}
      </section>
        ...
        <section aria-label="Alertas">
          ...
          <Cartao className="divide-y divide-borda">
            ...
            {painel.data.alertas.map((a) => {
              ...
              return (
                <div key={a.mensagem} className="flex items-center gap-3 p-4">
                  <span className={`rounded-full p-2 ${a.nivel === 'aviso' ? 'bg-alerta-suave text-alerta' : 'bg-primaria-suave text-primaria'}`}>
            ...
            {painel.data.modulosPendentes.length > 0 && (
              <TextoSuave className="p-4 text-xs">Em breve neste painel: {painel.data.modulosPendentes.join(' · ')}.</TextoSuave>
            )}
          </Cartao>
        </section>
      )}
