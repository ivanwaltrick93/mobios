 ...
 */
export function Etapas({ titulos, atual, aoIr, livre = false, comErro = [] }: { titulos: string[]; atual: number; aoIr: (i: number) => void; livre?: boolean; comErro?: number[] }) {
  return (
      ...
      {titulos.map((titulo, i) => {
        ...
        return (
          <li key={titulo} className="flex flex-1 items-start last:flex-none">
            ...
            >
              <span className={`flex size-9 items-center justify-center rounded-full border-2 text-sm font-semibold transition ${circulo} ${clicavel && i !== atual ? 'group-hover:border-primaria' : ''}`}>
                {feita && !erro ? <Check className="size-4" aria-hidden /> : i + 1}
              </span>
              <span className={`text-center text-xs ${i === atual ? 'font-semibold text-texto' : 'text-texto-suave'} max-sm:sr-only`}>{titulo}</span>
            </button>
            {i < titulos.length - 1 && <span className={`mx-2 mt-[18px] h-0.5 flex-1 rounded ${feita ? 'bg-primaria' : 'bg-borda'}`} aria-hidden />}
          </li>
        );
      })}
    </ol>
