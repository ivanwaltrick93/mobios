...
export function Avatar({
  ...
  return fotoVersao ? (
    ...
    <span
      aria-hidden
      className={`${classe} inline-flex items-center justify-center bg-primaria-suave font-semibold text-primaria`}
    >
      {iniciais(nome)}
    </span>
  );
...
export const Iniciais = ({ nome, tamanho = 'md' }: { nome: string; tamanho?: keyof typeof tamanhos }) => (
  <span
    aria-hidden
    className={`${tamanhos[tamanho]} inline-flex shrink-0 items-center justify-center rounded-full bg-primaria-suave font-semibold text-primaria`}
  >
    {iniciais(nome)}
  </span>
);
