import {
  ...
  hojeIso,
  idParamSchema,
  itemPrecoQuerySchema,
  itemTabelaQuerySchema,
...
const colunas = {
  ...
  servicoId: materiaisPrecos.servicoId,
...
/** Item pedido na consulta (`materialId` ou `servicoId`; o schema garante que vem só um). */
const itemConsultado = (q: { materialId?: string; servicoId?: string }): { tipo: TipoItemPreco; id: string } =>
  q.materialId ? { tipo: 'material', id: q.materialId } : { tipo: 'servico', id: q.servicoId! };
...
export const precosRoutes: FastifyPluginAsyncZod = async (app) => {
    ...
    async (req) => {
      ...
      return withTenant(req.user.tid, async (tx) => {
        ...
        const [linha] = await consulta(tx)
          .where(vigenteEm({ tipo: 'material', id: material.id }, tab.id, data))
          .limit(1);
  ...
  app.get(
    ...
    async (req) => {
      const { tabelaPrecoId, q, tipo, situacao, pagina, porPagina } = req.query;
      const hoje = hojeIso();
      ...
      const busca = q ? sql`and (${buscaDeMaterial(q)} or ${buscaDeServico(q)})` : sql``;
      const filtroTipo =
        tipo === 'material'
          ? sql`and l.material_id is not null`
          : tipo === 'servico'
            ? sql`and l.servico_id is not null`
            : sql``;
      // `materiais` e `servicos` sem apelido: os filtros de busca usam as colunas com o nome da tabela.
      const base = sql`
        from (
          select p.id::text as id, p.material_id, p.servico_id, p.preco_centavos, p.data_inicio, p.data_fim,
            ${situacaoDaVigencia} as situacao, 1 as ordem
          from materiais_precos p where p.tabela_preco_id = ${tabelaPrecoId}
          union all
          select 'padrao:' || coalesce(pp.material_id, pp.servico_id), pp.material_id, pp.servico_id,
            pp.preco_centavos, null, null, 'padrao', 2
          from precos_padrao pp where pp.tabela_preco_id = ${tabelaPrecoId}
        ) l
        left join materiais on materiais.id = l.material_id
        left join servicos on servicos.id = l.servico_id
        where coalesce(materiais.ativo, servicos.ativo) ${filtroTipo} ${busca} ${filtroSituacao}`;
      return withTenant(req.user.tid, async (tx) => {
        ...
        const linhas = (await tx.execute(sql`
          select l.id,
            case when l.material_id is not null then 'material' else 'servico' end as tipo,
            coalesce(l.material_id, l.servico_id) as "itemId",
            materiais.sku, servicos.codigo as "servicoCodigo",
            coalesce(materiais.descricao, servicos.nome) as descricao,
            servicos.forma_preco as "formaPreco",
            l.preco_centavos::float8 as "precoCentavos",
            l.data_inicio::text as "dataInicio",
            l.data_fim::text as "dataFim",
            l.situacao
          ${base}
          order by coalesce(materiais.descricao, servicos.nome), materiais.sku, servicos.codigo, l.ordem, l.data_inicio
          limit ${porPagina} offset ${(pagina - 1) * porPagina}`)) as unknown as (Omit<LinhaPreco, 'codigo'> & {
          sku: string | null;
          servicoCodigo: number | null;
        })[];
        const itens: LinhaPreco[] = linhas.map(({ sku, servicoCodigo, ...l }) => ({
          ...l,
          codigo: sku ?? formatarCodigoServico(servicoCodigo!),
        }));
  ...
  /**
   * Todas as vigências do material ou serviço (histórico completo, inclusive canceladas), por tabela e da mais
   * nova para a mais antiga.
   */
  app.get(
    '/',
    { schema: { querystring: itemPrecoQuerySchema, response: { 200: z.array(precoSchema) } } },
    async (req) =>
      withTenant(req.user.tid, async (tx) => {
        const lista = await consulta(tx)
          .where(
            and(
              doItem(materiaisPrecos, itemConsultado(req.query)),
  ...
  /** Nova vigência (regras em criarVigencia). Material pelo id ou SKU; serviço pelo id ou código. */
  app.post(
    '/',
    { ...editar, schema: { body: precoInputSchema, response: { 201: precoSchema } } },
    async (req, reply) => {
      const { materialId, sku, servicoId, servicoCodigo, ...vigencia } = req.body;
      const preco = await withTenant(req.user.tid, async (tx) => {
        const item = await resolverItem(tx, { materialId, sku, servicoId, servicoCodigo });
        const id = await criarVigencia(tx, { ...vigencia, item }, req.user.sub);
  ...
  app.put(
    ...
    async (req) =>
      withTenant(req.user.tid, async (tx) => {
        ...
        await travar(tx, itemDaLinha(p).id, p.tabelaPrecoId);
  ...
  /** Preços padrão do material ou serviço, um por tabela. */
  app.get(
    '/padrao',
    { schema: { querystring: itemPrecoQuerySchema, response: { 200: z.array(precoPadraoSchema) } } },
    async (req) =>
      withTenant(req.user.tid, (tx) =>
        consultaPadrao(tx)
          .where(
            and(
              doItem(precosPadrao, itemConsultado(req.query)),
              req.query.tabelaPrecoId ? eq(precosPadrao.tabelaPrecoId, req.query.tabelaPrecoId) : undefined,
            ),
          )
  ...
  /** Define ou altera o preço padrão. Material pelo id ou SKU; serviço pelo id ou código; `versao` protege a edição. */
  app.put(
    '/padrao',
    { ...editar, schema: { body: precoPadraoInputSchema, response: { 200: precoPadraoSchema } } },
    async (req) => {
      const { materialId, sku, servicoId, servicoCodigo, ...dados } = req.body;
      return withTenant(req.user.tid, async (tx) => {
        const item = await resolverItem(tx, { materialId, sku, servicoId, servicoCodigo });
        await definirPrecoPadrao(tx, { ...dados, item }, req.user.sub);
        const [padrao] = await consultaPadrao(tx).where(
          and(doItem(precosPadrao, item), eq(precosPadrao.tabelaPrecoId, dados.tabelaPrecoId)),
        );
        return padrao!;
      });
    },
  ...
  app.delete('/padrao', { ...editar, schema: { querystring: itemTabelaQuerySchema } }, async (req, reply) => {
    await withTenant(req.user.tid, (tx) =>
      removerPrecoPadrao(tx, itemConsultado(req.query), req.query.tabelaPrecoId, req.user.sub),
    );
    return reply.code(204).send();
  });

  /** Trilha do preço padrão de um material ou serviço numa tabela (mais recente primeiro). */
  app.get(
    '/padrao/eventos',
    { schema: { querystring: itemTabelaQuerySchema, response: { 200: z.array(eventoPrecoPadraoSchema) } } },
    async (req) =>
      withTenant(req.user.tid, (tx) =>
        tx
          .select({
          ...
          .where(
            and(
              doItem(precosPadraoEventos, itemConsultado(req.query)),
   ...
   * `tipo` (material ou servico; vazio = material) e `codigo` (SKU ou código do serviço; `sku` também vale).
  ...
  app.post(
    ...
    async (req) => {
      ...
      return withTenant(req.user.tid, async (tx) => {
        // Tabelas, materiais e serviços da planilha lidos de uma vez, não uma consulta por linha.
        ...
        const codigoDa = (valores: Record<string, string>) =>
          (valores.codigo || valores.sku || '').trim().toUpperCase();
        const ehServico = (valores: Record<string, string>) => comparavel(valores.tipo ?? '') === 'servico';
        const skus = [...new Set(linhas.filter((l) => !ehServico(l.valores)).map((l) => codigoDa(l.valores)))];
        const codigosServico = [
          ...new Set(linhas.filter((l) => ehServico(l.valores)).map((l) => Number(codigoDa(l.valores)))),
        ...
        const encontrados = skus.length
          ...
          : [];
        const porSku = new Map(encontrados.map((m) => [m.sku, { tipo: 'material' as const, ...m }]));
        const servicosEncontrados = codigosServico.length
          ? await tx
              .select({ id: servicos.id, codigo: servicos.codigo, ativo: servicos.ativo })
              .from(servicos)
              .where(inArray(servicos.codigo, codigosServico))
          : [];
        const porCodigoServico = new Map(
          servicosEncontrados.map((sv) => [sv.codigo, { tipo: 'servico' as const, ...sv }]),
        );
        ...
        return importarLinhas(tx, linhas, async (savepoint, { valores }) => {
          ...
          await criarVigencia(savepoint, { item, tabelaPrecoId, precoCentavos, dataInicio, dataFim }, req.user.sub);
          return 'importada';
        });
      });
    },
...
const consultaPadrao = (tx: Tx) =>
  tx
    .select({
      ...
      servicoId: precosPadrao.servicoId,
