...
const comSituacao = <T extends Linha>({ cancelado, ...p }: T, data = hojeIso()) => ({ ...p, situacao: situacaoPreco({ ...p, cancelado }, data) });

const consulta = (tx: Tx) => tx.select(colunas).from(materiaisPrecos).innerJoin(tabelasPreco, eq(tabelasPreco.id, materiaisPrecos.tabelaPrecoId));
...
const registrar = (tx: Tx, precoId: string, evento: Evento, usuarioId: string, antes: object | null, depois: object | null) =>
  tx.insert(precosEventos).values({ precoId, evento, usuarioId, antes, depois });

/** Campos da vigência que entram na trilha de auditoria. */
const retrato = (p: { precoCentavos: number; dataInicio: string; dataFim: string | null }) => ({ precoCentavos: p.precoCentavos, dataInicio: p.dataInicio, dataFim: p.dataFim });
...
export const precosRoutes: FastifyPluginAsyncZod = async (app) => {
   ...
   */
  app.get('/vigente', { schema: { querystring: precoVigenteQuerySchema, response: { 200: precoVigenteSchema } } }, async (req) => {
    ...
    return withTenant(req.user.tid, async (tx) => {
      ...
      const [tab] = await tx
        .select({ id: tabelasPreco.id, codigo: tabelasPreco.codigo, nome: tabelasPreco.nome, ativa: tabelasPreco.ativa })
        .from(tabelasPreco)
      ...
      const [preco] = await consulta(tx).where(vigenteEm(material.id, tab.id, data)).limit(1);
  ...
  app.get(
    '/lista',
    { schema: { querystring: listaPrecosQuerySchema, response: { 200: z.object({ itens: z.array(itemListaPrecosSchema), total: z.number() }) } } },
    async (req) => {
      ...
      return withTenant(req.user.tid, async (tx) => {
        const [tabela] = await tx.select({ id: tabelasPreco.id }).from(tabelasPreco).where(eq(tabelasPreco.id, tabelaPrecoId));
        ...
        const [{ total }] = (await tx.execute(sql`select count(*)::int as total ${base}`)) as unknown as [{ total: number }];
  ...
  app.get(
    '/',
    { schema: { querystring: z.object({ materialId: z.uuid(), tabelaPrecoId: z.uuid().optional() }), response: { 200: z.array(precoSchema) } } },
    async (req) =>
      withTenant(req.user.tid, async (tx) => {
        const lista = await consulta(tx)
          .where(and(eq(materiaisPrecos.materialId, req.query.materialId), req.query.tabelaPrecoId ? eq(materiaisPrecos.tabelaPrecoId, req.query.tabelaPrecoId) : undefined))
   ...
   */
  app.post('/', { ...editar, schema: { body: precoInputSchema, response: { 201: precoSchema } } }, async (req, reply) => {
    const { materialId, tabelaPrecoId, precoCentavos, dataInicio } = req.body;
    let { dataFim } = req.body;
    const hoje = hojeIso();
    if (dataInicio < hoje) throw new ErroHttp(400, 'A vigência não pode começar no passado: o histórico de preços não é reescrito.');

    const preco = await withTenant(req.user.tid, async (tx) => {
      await travar(tx, materialId, tabelaPrecoId);
      const [material] = await tx.select({ ativo: materiais.ativo }).from(materiais).where(eq(materiais.id, materialId));
      if (!material) throw naoEncontrado('Material');
      if (!material.ativo) throw new ErroHttp(400, 'Material inativo não recebe preço novo. Reative-o primeiro.');
      const [tabela] = await tx.select({ ativa: tabelasPreco.ativa }).from(tabelasPreco).where(eq(tabelasPreco.id, tabelaPrecoId));
      if (!tabela) throw naoEncontrado('Tabela de preço');
      if (!tabela.ativa) throw new ErroHttp(400, 'Tabela de preço inativa não recebe preço novo. Reative-a primeiro.');

      const [atual] = await tx.select().from(materiaisPrecos).where(vigenteEm(materialId, tabelaPrecoId, dataInicio));
      if (atual?.dataInicio === dataInicio) {
        throw new ErroHttp(409, 'Já existe um preço começando nesta data. Edite-o (se ainda não começou) ou escolha outra data de início.');
      }
      const [proximo] = await tx
        .select({ dataInicio: materiaisPrecos.dataInicio })
        .from(materiaisPrecos)
        .where(and(eq(materiaisPrecos.materialId, materialId), eq(materiaisPrecos.tabelaPrecoId, tabelaPrecoId), not(materiaisPrecos.cancelado), gt(materiaisPrecos.dataInicio, dataInicio)))
      ...
      if (proximo) {
        if (!dataFim) dataFim = diaAnterior(proximo.dataInicio);
          ...
          throw new ErroHttp(409, `Já existe preço programado a partir de ${proximo.dataInicio.split('-').reverse().join('/')}. Termine a nova vigência antes dessa data.`);
        }
      }
      ...
      if (atual && dataFim && (atual.dataFim === null || dataFim < atual.dataFim)) {
        ...
        throw new ErroHttp(409, `O período se sobrepõe ao preço vigente (${fimAtual}). Deixe a nova vigência sem fim ou termine-a depois do fim da atual.`);
      }
      ...
      const [novo] = (await tx
        ...
        .values({ materialId, tabelaPrecoId, precoCentavos, dataInicio, dataFim: dataFim ?? null, criadoPor: req.user.sub, atualizadoPor: req.user.sub })
      ...
      if (atual) {
        ...
        await registrar(tx, atual.id, 'encerrado', req.user.sub, retrato(atual), { ...retrato(atual), dataFim: diaAnterior(dataInicio), motivo: 'Nova vigência' });
      }
  ...
  app.put('/:id', { ...editar, schema: { params: idParamSchema, body: precoAtualizarSchema, response: { 200: precoSchema } } }, async (req) =>
    withTenant(req.user.tid, async (tx) => {
      ...
      if (atual.dataInicio <= hojeIso()) {
        throw new ErroHttp(409, 'Este preço já está em vigor (ou encerrado) e não pode ser alterado. Cadastre uma nova vigência.');
      }
      const { precoCentavos, dataFim } = req.body;
      if (dataFim && dataFim < atual.dataInicio) throw new ErroHttp(400, 'O fim deve ser igual ou posterior ao início.');
      await tx.update(materiaisPrecos).set({ precoCentavos, dataFim, atualizadoPor: req.user.sub }).where(eq(materiaisPrecos.id, atual.id));
      await registrar(tx, atual.id, 'alterado', req.user.sub, retrato(atual), { ...retrato(atual), precoCentavos, dataFim });
      return carregar(tx, atual.id);
    }),
  );

  /** Encerra (ou reprograma o fim de) uma vigência atual ou futura. Não mexe em dia que já passou. */
  app.post('/:id/encerrar', { ...editar, schema: { params: idParamSchema, body: precoEncerrarSchema, response: { 200: precoSchema } } }, async (req) =>
    withTenant(req.user.tid, async (tx) => {
      const p = await buscarPreco(tx, req.params.id);
      await travar(tx, p.materialId, p.tabelaPrecoId);
      ...
      await tx.update(materiaisPrecos).set({ dataFim, dataFimAnterior: null, encerradoPeloPrecoId: null, atualizadoPor: req.user.sub }).where(eq(materiaisPrecos.id, atual.id));
   ...
   */
  app.post('/:id/cancelar', { ...editar, schema: { params: idParamSchema, body: precoCancelarSchema, response: { 200: precoSchema } } }, async (req) =>
    withTenant(req.user.tid, async (tx) => {
      const p = await buscarPreco(tx, req.params.id);
      await travar(tx, p.materialId, p.tabelaPrecoId);
      const atual = await buscarPreco(tx, req.params.id);
      if (atual.cancelado) throw new ErroHttp(409, 'Este preço já está cancelado.');
      if (atual.dataInicio <= hojeIso()) throw new ErroHttp(409, 'Só é possível cancelar preço que ainda não começou. Para um preço em vigor, use Encerrar.');
      await tx
        .update(materiaisPrecos)
        .set({ cancelado: true, motivoCancelamento: req.body.motivo, canceladoEm: new Date(), canceladoPor: req.user.sub, atualizadoPor: req.user.sub })
        .where(eq(materiaisPrecos.id, atual.id));
      await registrar(tx, atual.id, 'cancelado', req.user.sub, retrato(atual), { motivo: req.body.motivo });

      const encerrados = await tx.select().from(materiaisPrecos).where(eq(materiaisPrecos.encerradoPeloPrecoId, atual.id));
      for (const anterior of encerrados) {
        await tx
          .update(materiaisPrecos)
          .set({ dataFim: anterior.dataFimAnterior, dataFimAnterior: null, encerradoPeloPrecoId: null, atualizadoPor: req.user.sub })
          .where(eq(materiaisPrecos.id, anterior.id));
        await registrar(tx, anterior.id, 'reaberto', req.user.sub, retrato(anterior), { ...retrato(anterior), dataFim: anterior.dataFimAnterior });
      }
      return carregar(tx, atual.id);
    }),
  );
  ...
  app.get(
    '/:id/eventos',
    {
      schema: {
        ...
        response: {
          200: z.array(z.object({ evento: z.string(), antes: z.unknown(), depois: z.unknown(), usuario: z.string().nullable(), criadoEm: z.coerce.date() })),
        },
      },
    },
    async (req) =>
      withTenant(req.user.tid, async (tx) => {
        ...
        return tx
          .select({ evento: precosEventos.evento, antes: precosEventos.antes, depois: precosEventos.depois, usuario: nomeUsuario('precos_eventos', 'usuario_id'), criadoEm: precosEventos.criadoEm })
          .from(precosEventos)
