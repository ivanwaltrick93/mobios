    ...
    withTenant(req.user.tid, async (tx) => {
      const [c] = await tx
        .select({
          ...
          incompletos:
            sql<number>`count(*) filter (where ${clientes.cpfCnpj} is null or ${clientes.telefone} is null or ${clientes.whatsapp} is null
            or not exists (select 1 from cliente_enderecos e where e.cliente_id = "clientes"."id")
            or (${clientes.tipo} = 'PJ' and not exists (select 1 from cliente_responsaveis r where r.cliente_id = "clientes"."id")))`.mapWith(
              Number,
            ),
          ...
          semVeiculo:
            sql<number>`count(*) filter (where not exists (select 1 from veiculos v where v.cliente_id = "clientes"."id"))`.mapWith(
              Number,
            ),
        })
        .from(clientes);
      const [v] = await tx
        .select({
          ...
          incompletos:
            sql<number>`count(*) filter (where ${veiculos.anoFabricacao} is null or ${veiculos.anoModelo} is null)`.mapWith(
              Number,
            ),
        })
        .from(veiculos);
      ...
      const indicadores: Indicador[] = [
          ...
          : []),
        {
          ...
          valor: c!.total,
          formato: 'numero',
          detalhe: `${c!.hoje} cadastrado(s) hoje`,
          link: '/clientes',
        },
        {
          id: 'veiculos',
          ...
          valor: v!.total,
          formato: 'numero',
          detalhe: `${v!.hoje} cadastrado(s) hoje`,
          link: '/clientes',
        },
      ];
