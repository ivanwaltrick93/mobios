...
import { alterarAtivo, atualizarVersionado, excluirSeNaoUsado, exigirVersao, nomeUsuario, validarReferencia } from '../../lib/cadastro.js';
...
const consulta = (tx: Tx) => tx.select(colunas).from(depositos).innerJoin(tiposDeposito, eq(tiposDeposito.id, depositos.tipoId));
...
export const depositosRoutes: FastifyPluginAsyncZod = async (app) => {
  ...
  app.get('/', { schema: { response: { 200: z.array(depositoSchema) } } }, async (req) => withTenant(req.user.tid, (tx) => consulta(tx).orderBy(asc(depositos.nome))));
  app.get('/:id', { schema: { params: idParamSchema, response: { 200: depositoSchema } } }, async (req) => withTenant(req.user.tid, (tx) => carregar(tx, req.params.id)));

  app.post('/', { ...editar, schema: { body: depositoInputSchema, response: { 201: depositoSchema } } }, async (req, reply) => {
    ...
    const deposito = await withTenant(req.user.tid, async (tx) => {
      ...
      const [{ id }] = (await tx.insert(depositos).values({ ...dados, criadoPor: req.user.sub, atualizadoPor: req.user.sub }).returning({ id: depositos.id })) as [{ id: string }];
      return carregar(tx, id);
    });
    return reply.code(201).send(deposito);
  });

  app.put('/:id', { ...editar, schema: { params: idParamSchema, body: depositoInputSchema, response: { 200: depositoSchema } } }, async (req) => {
    ...
    return withTenant(req.user.tid, async (tx) => {
      ...
      await atualizarVersionado(tx, depositos, req.params.id, exigirVersao(versao), { ...dados, atualizadoPor: req.user.sub }, 'Depósito');
      return carregar(tx, req.params.id);
    });
  });

  app.patch('/:id/status', { ...editar, schema: { params: idParamSchema, body: statusInputSchema, response: { 200: depositoSchema } } }, async (req) =>
  ...
  app.delete('/:id', { ...editar, schema: { params: idParamSchema } }, async (req, reply) => {
    await withTenant(req.user.tid, (tx) => excluirSeNaoUsado(tx, depositos, req.params.id, 'Depósito', 'Este depósito está em uso e não pode ser excluído. Inative-o.'));
