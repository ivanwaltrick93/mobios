import { idParamSchema, listaOpcoesSchema, LISTAS_OPCOES, opcaoInputSchema, opcaoSchema, temAcesso, type ListaOpcoes } from '@mobios/shared';
import { asc, eq, sql } from 'drizzle-orm';
import type { FastifyPluginAsyncZod } from 'fastify-type-provider-zod';
import { z } from 'zod';
import { withTenant } from '../../db/client.js';
import { cargosResponsavel, origensCliente, relacionamentosCliente, tiposDeposito, tiposMaterial } from '../../db/schema.js';
...
export const opcoesRoutes: FastifyPluginAsyncZod = async (app) => {
  ...
  app.get(
    '/:lista',
    { schema: { params: listaParam, response: { 200: z.array(opcaoSchema) } } },
    async (req) => {
      // Consulta liberada para quem acessa o módulo que usa a lista (clientes ou materiais).
      if (!temAcesso(req.user.acessos, LISTAS_OPCOES[req.params.lista].modulo)) throw new ErroHttp(403, 'Você não tem permissão para esta ação.');
