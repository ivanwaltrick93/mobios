import { ajusteEstoqueSchema, estoqueAjusteSchema, estoqueFiltroSchema, saldoSchema, UNIDADES, type Saldo } from '@mobios/shared';
...
export const estoqueRoutes: FastifyPluginAsyncZod = async (app) => {
   ...
   */
  app.get('/', { schema: { querystring: estoqueFiltroSchema, response: { 200: z.object({ itens: z.array(saldoSchema), total: z.number() }) } } }, async (req) => {
    ...
    const filtros: (SQL | undefined)[] = [
      ...
      or(sql`${estoques.materialId} is not null`, and(eq(materiais.ativo, true), eq(materiais.controlaEstoque, true), eq(depositos.ativo, true))),
      q ? or(ilike(materiais.sku, `${q.toUpperCase()}%`), ilike(materiais.descricao, `%${q}%`), ilike(materiais.codigoFabricante, `${q.toUpperCase()}%`), eq(materiais.codigoBarras, q)) : undefined,
  ...
  app.get('/material/:materialId', { schema: { params: z.object({ materialId: z.uuid() }), response: { 200: z.array(saldoSchema) } } }, async (req) =>
    withTenant(req.user.tid, async (tx) => {
      const lista = await tx
        ...
        .innerJoin(depositos, sql`true`)
        .leftJoin(estoques, and(eq(estoques.materialId, materiais.id), eq(estoques.depositoId, depositos.id)))
        .where(and(eq(materiais.id, req.params.materialId), or(eq(depositos.ativo, true), sql`${estoques.materialId} is not null`)))
        .orderBy(asc(depositos.codigo));
      if (!lista.length) {
        const [existe] = await tx.select({ id: materiais.id }).from(materiais).where(eq(materiais.id, req.params.materialId));
   ...
   */
  app.put('/:materialId/:depositoId', { ...editar, schema: { params: chaveParams, body: estoqueAjusteSchema, response: { 200: saldoSchema } } }, async (req) => {
    ...
    return withTenant(req.user.tid, async (tx) => {
      ...
      if (!material.controlaEstoque) throw new ErroHttp(400, 'Este material não controla estoque (veja os controles do cadastro).');
      ...
      const [deposito] = await tx.select({ ativo: depositos.ativo }).from(depositos).where(eq(depositos.id, depositoId));
      ...
      if (atual) {
        ...
        if (versao !== atual.versao) throw new ErroHttp(409, 'O saldo foi alterado por outra pessoa enquanto você editava. Recarregue e refaça o ajuste.');
        if (atual.disponivel === disponivel && atual.reservado === reservado) throw new ErroHttp(400, 'Nenhuma quantidade foi alterada.');
        await tx
          .update(estoques)
        ...
        await tx.insert(estoques).values({ materialId, depositoId, disponivel, reservado, atualizadoPor: req.user.sub });
      }
  ...
  app.get('/:materialId/:depositoId/ajustes', { schema: { params: chaveParams, response: { 200: z.array(ajusteEstoqueSchema) } } }, async (req) =>
    withTenant(req.user.tid, (tx) =>
      tx
        ...
        .from(estoqueAjustes)
        .where(and(eq(estoqueAjustes.materialId, req.params.materialId), eq(estoqueAjustes.depositoId, req.params.depositoId)))
