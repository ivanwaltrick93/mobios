...
async function listar(tx: Tx, id?: string): Promise<Usuario[]> {
  const lista = await tx
    .select({ id: users.id, nome: users.nome, email: users.email, ativo: users.ativo, criadoEm: users.criadoEm, fotoEm: usuarioFotos.atualizadoEm })
    .from(users)
  ...
  const vinculos = lista.length
    ? await tx
        .select({ usuarioId: usuarioFuncoes.usuarioId, id: funcoes.id, nome: funcoes.nome, admin: funcoes.admin, ativa: funcoes.ativa })
        .from(usuarioFuncoes)
        .innerJoin(funcoes, eq(funcoes.id, usuarioFuncoes.funcaoId))
        .where(inArray(usuarioFuncoes.usuarioId, lista.map((u) => u.id)))
        .orderBy(asc(funcoes.nome))
    : [];
  return lista.map(({ fotoEm, ...u }) => ({ ...u, fotoVersao: versaoFoto(fotoEm), funcoes: vinculos.filter((v) => v.usuarioId === u.id).map(({ usuarioId: _, ...f }) => f) }));
}
...
async function trocarFuncoes(tx: Tx, usuarioId: string, funcaoIds: string[]) {
  ...
  await tx.delete(usuarioFuncoes).where(and(eq(usuarioFuncoes.usuarioId, usuarioId), inArray(usuarioFuncoes.funcaoId, ativas)));
...
export const usuariosRoutes: FastifyPluginAsyncZod = async (app) => {
  ...
  app.get('/', { schema: { response: { 200: z.array(usuarioSchema) } } }, async (req) => withTenant(req.user.tid, (tx) => listar(tx)));
  ...
  app.post('/', { schema: { body: usuarioCriarSchema, response: { 201: usuarioSchema } } }, async (req, reply) => {
    ...
    const usuario = await withTenant(req.user.tid, async (tx) => {
      const [criado] = await tx.insert(users).values({ ...dados, senhaHash }).returning({ id: users.id });
  ...
  app.put('/:id', { schema: { params: idParamSchema, body: usuarioAtualizarSchema, response: { 200: usuarioSchema } } }, async (req) => {
    ...
    return withTenant(req.user.tid, async (tx) => {
      ...
      if (req.params.id === req.user.sub) {
        ...
        if (!incluiAdmin || !dados.ativo) throw new ErroHttp(400, 'Você não pode desativar a sua conta nem retirar dela a função Administrador.');
      }
